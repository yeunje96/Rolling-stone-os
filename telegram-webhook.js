...
${ctx}`;
    ...
    try {
      const apiRes = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'gpt-4o', max_tokens: 500, messages: [{ role: 'system', content: systemPrompt }, ...[
            ...history.slice(-12).map(c => ({ role: c.role, content: c.content })),
            { role: 'user', content: actualMessage }
          ]] })
      });

      if (!apiRes.ok) throw new Error('API 오류: ' + await apiRes.text());
      const apiData = await apiRes.json();
      const reply = apiData.choices?.[0]?.message?.content?.trim() || '처리 중 오류가 발생했습니다.';
